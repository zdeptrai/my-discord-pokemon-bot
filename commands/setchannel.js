...
module.exports = {
    name: 'setchannel',
    description: 'Thiết lập hoặc xóa kênh được phép cho một loại lệnh cụ thể (market, battle, spawn, boss, welcome).', 
    aliases: ['sc', 'channelconfig'],
    usage: '<market|battle|spawn|boss|welcome> <add|remove|list|clear> [channel_id]', 
    cooldown: 5,
    ...
    async execute(message, args, client) {
        ...
        if (!hasAdminPerms) {
            ...
            }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn thiếu quyền cho ${userId}:`, e));
            return;
        }
        ...
        if (args.length < 1 || (args[0] !== 'list' && args[0] !== 'clear' && args.length < 2)) {
            await message.channel.send({
                ...
                flags: MessageFlags.Ephemeral
            }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn cú pháp cho ${userId}:`, e));
            return;
        }
        ...
        if (args.length === 1) {
            if (channelType === 'list' || channelType === 'clear') {
                await message.channel.send({
                    ...
                    flags: MessageFlags.Ephemeral
                }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn thiếu loại kênh cho ${userId}:`, e));
                return;
            }
            action = 'add';
        } else {
            ...
            if (action === 'add' || action === 'remove') {
                    ...
                    if (mentionedChannel) {
                        ...
                        if (parsedId && parsedId.length >= 17) {
                            ...
                            await message.channel.send({
                                ...
                                flags: MessageFlags.Ephemeral
                            }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn ID kênh không hợp lệ cho ${userId}:`, e));
                            return;
                        }
                    }
                ...
                await message.channel.send({
                    ...
                    flags: MessageFlags.Ephemeral
                }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn hành động không hợp lệ cho ${userId}:`, e));
                return;
            }
        }
        ...
        if (!dbColumn) {
            await message.channel.send({
                content: `<@${userId}> Loại kênh không hợp lệ. Vui lòng chọn một trong số: \`market\`, \`battle\`, \`spawn\`, \`boss\`, \`welcome\`.`, 
                flags: MessageFlags.Ephemeral
            }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi tin nhắn loại kênh không hợp lệ cho ${userId}:`, e));
            return;
        }
        ...
        try {
            ...
            let updated = false;
            // Khai báo channelIds ở đây để nó có thể truy cập được ở cuối hàm
            let channelIds = []; 

            // Xử lý riêng cho các cột đơn (boss_channel_id, welcome_channel_id)
            if (channelType === 'boss' || channelType === 'welcome') { 
                ...
                if (guildSettings && guildSettings[dbColumn]) {
                    try {
                        ...
                        sendOwnerDM(client, `[Lỗi SetChannel] Lỗi phân tích JSON cho ${dbColumn} trong guild ${guildId}.`, e);
                        channelIds = [];
                    }
                }
            ...
            if (action !== 'list') { // Chỉ thêm field này nếu không phải hành động 'list'
                let currentValue;
                if (channelType === 'boss' || channelType === 'welcome') { 
                    ...
                    // Sử dụng biến channelIds đã được cập nhật hoặc khởi tạo
            ...
            await message.channel.send({ embeds: [embed] }).catch(e => {
                console.error(`[SETCHANNEL_ERROR] Lỗi gửi embed kết quả cho ${userId}:`, e);
                sendOwnerDM(client, `[Lỗi SetChannel] Lỗi gửi embed kết quả cho ${userId}.`, e);
            });

        } catch (error) {
            console.error(`[SETCHANNEL_COMMAND_ERROR] Lỗi khi thiết lập kênh ${channelType}:`, error);
            sendOwnerDM(client, `[Lỗi SetChannel] Lỗi khi thiết lập kênh ${channelType} cho guild ${guildId}.`, error);
            await message.channel.send({
                content: `<@${userId}> Đã có lỗi xảy ra khi thiết lập kênh. Vui lòng thử lại sau.`,
                flags: MessageFlags.Ephemeral
            }).catch(e => console.error(`[SETCHANNEL_ERROR] Lỗi gửi phản hồi lỗi chung cho ${userId}:`, e));
        }
    },
};
