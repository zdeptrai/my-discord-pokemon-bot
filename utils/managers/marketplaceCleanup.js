...
async function cleanupExpiredListings(client) {
    // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
    logger.pokemon('MARKETPLACE_CLEANUP', 'Bắt đầu kiểm tra các listing hết hạn...');
    const now = new Date();
    ...
    try {
        ...
        if (expiredListings.length === 0) {
            // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
            logger.pokemon('MARKETPLACE_CLEANUP', 'Không có listing nào hết hạn.');
            return;
        }

        // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
        logger.pokemon('MARKETPLACE_CLEANUP', `Tìm thấy ${expiredListings.length} listing hết hạn.`);
            ...
            await db.transaction(async trx => {
                try {
                    ...
                    if (listing.item_type === 'pokemon') {
                        ...
                        if (updatedCount > 0) {
                            ...
                            if (userPokemon) {
                                ...
                                returnSuccess = true;
                                // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
                                logger.pokemon('MARKETPLACE_CLEANUP', `Đã trả lại Pokémon ${itemDisplayName} (ID: ${listing.item_reference_id}) cho người bán ${sellerId}.`);
                            }
                        } else {
                            // Thay thế console.warn bằng logger.pokemon, bỏ bớt dấu [] trong tag
                            logger.pokemon('MARKETPLACE_CLEANUP_WARN', `Không tìm thấy user_pokemon với ID ${listing.item_reference_id} để trả lại cho listing ${listing.listing_id}.`);
                        }
                        ...
                        if (updatedCount === 0) {
                            ...
                            // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
                            logger.pokemon('MARKETPLACE_CLEANUP', `Đã tạo bản ghi inventory mới và trả lại item ${listing.item_reference_id} (x${listing.quantity}) cho người bán ${sellerId}.`);
                        } else {
                            // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
                            logger.pokemon('MARKETPLACE_CLEANUP', `Đã tăng số lượng item ${listing.item_reference_id} (x${listing.quantity}) trong inventory của người bán ${sellerId}.`);
                        }
                        ...
                        if (itemDetails) {
                            ...
                            // Thay thế console.warn bằng logger.warn, bỏ bớt dấu [] trong tag
                    ...
                    await trx('marketplace_listings')
                        ...
                        .del();
                    // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
                    logger.pokemon('MARKETPLACE_CLEANUP', `Đã xóa listing ${listing.listing_id} khỏi database.`);
                    ...
                    if (returnSuccess) {
                        try {
                            ...
                            // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
                            logger.pokemon('MARKETPLACE_CLEANUP', `Đã xử lý và gửi DM cho người bán ${sellerId} về listing ${listing.listing_id}.`);
                        } catch (dmError) {
                            // Thay thế console.error bằng logger.error, bỏ bớt dấu [] trong tag
                            logger.error('MARKETPLACE_CLEANUP_DM_ERROR', `Không thể gửi DM cho người bán ${sellerId} về listing ${listing.listing_id}:`, dmError);
                        }
                    } else {
                        // Thay thế console.warn bằng logger.warn, bỏ bớt dấu [] trong tag
                        logger.warn('MARKETPLACE_CLEANUP_WARN', `Không thể trả lại vật phẩm/Pokémon cho listing ${listing.listing_id}.`);
                    }

                } catch (innerError) {
                    // Thay thế console.error bằng logger.error, bỏ bớt dấu [] trong tag
                    logger.error('MARKETPLACE_CLEANUP_TRANSACTION_ERROR', `Lỗi khi xử lý listing ${listing.listing_id} trong transaction:`, innerError);
                }
            });
        }
    } catch (error) {
        // Thay thế console.error bằng logger.error, bỏ bớt dấu [] trong tag
        logger.error('MARKETPLACE_CLEANUP_ERROR', 'Lỗi tổng quát trong quá trình dọn dẹp listing:', error);
    }
    // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
    logger.pokemon('MARKETPLACE_CLEANUP', 'Kết thúc kiểm tra các listing hết hạn.');
}

function scheduleCleanup(client) {
    // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
    logger.pokemon('MARKETPLACE_CLEANUP_MANAGER', 'Hàm scheduleCleanup đang được gọi.');
    if (cleanupIntervalId) {
        clearInterval(cleanupIntervalId);
        logger.pokemon('MARKETPLACE_CLEANUP_MANAGER', 'Tác vụ dọn dẹp marketplace đã được xóa bỏ trước khi lên lịch lại.');
    }
    cleanupIntervalId = setInterval(() => cleanupExpiredListings(client), CLEANUP_INTERVAL_MS);
    // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
    logger.pokemon('MARKETPLACE_CLEANUP_MANAGER', `Tác vụ dọn dẹp marketplace đã được lên lịch chạy mỗi ${CLEANUP_INTERVAL_MS / (1000 * 60)} phút.`);
}
...
function stopCleanup() {
    if (cleanupIntervalId) {
        ...
        // Thay thế console.log bằng logger.pokemon, bỏ bớt dấu [] trong tag
        logger.pokemon('MARKETPLACE_CLEANUP_MANAGER', 'Tác vụ dọn dẹp marketplace đã dừng.');
    }
}
